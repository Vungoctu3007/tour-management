...
public class SecurityConfig {
  ...
  private final String[] PUBLIC_ENDPOINT = {"/api/auth/**", "/api/route/**", "/api/admin/feedback/client", "/api/admin/feedback/comment", "/api/admin/feedback/checkBooking", "/api/customer/**", "/api/booking/**", "/api/vnpay/**", "/api/payment/**"};
  private final String[] PRIVATE_ENDPOINT = {"/api/admin/user/**", "/api/admin/role/**", "/api/admin/decentralization/**", "/api/admin/feedback/admin", "/api/payment/**"};

    @Bean
    public SecurityFilterChain filterChain(HttpSecurity httpSecurity,
            MyAuthorizationManager myAuthorizationManager) throws Exception {
      httpSecurity
              .cors(cors -> cors.configurationSource(
                      corsConfigurationSource())) // Thêm CORS vào HttpSecurity
              .authorizeHttpRequests(request -> request
                      .requestMatchers(PUBLIC_ENDPOINT).permitAll() // Cho phép truy cập không cần xác thực
                      .requestMatchers(PRIVATE_ENDPOINT).hasAnyRole("STAFF", "ADMIN")
